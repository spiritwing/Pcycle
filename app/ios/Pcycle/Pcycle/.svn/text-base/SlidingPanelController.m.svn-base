//
//  SlidingPanelController.m
//  Pcycle
//
//  Created by 郝 源顺 on 14-3-19.
...
@property (nonatomic, readwrite) SlidingPanelState state;
...
@implementation SlidingPanelController
...
- (void)viewDidLoad {
    ...
    self.state = SlidingPanelCenterVisible;
...
- (void)willAnimateRotationToInterfaceOrientation:(__unused UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    ...
    if (self.centerPanelHidden) {
        ...
        frame.origin.x = self.state == SlidingPanelLeftVisible ? self.centerPanelContainer.frame.size.width : -self.centerPanelContainer.frame.size.width;
...
- (void)setState:(SlidingPanelState)state {
    if (state != _state) {
        _state = state;
        switch (_state) {
            case SlidingPanelCenterVisible: {
...
- (void)setStyle:(SlidingPanelStyle)style {
    if (style != _style) {
        _style = style;
...
- (void)_layoutSideContainers:(BOOL)animate duration:(NSTimeInterval)duration {
    ...
    if (self.style == SlidingPanelMultipleActive) {
...
- (void)setCenterPanel:(UIViewController *)centerPanel {
    ...
    if (centerPanel != _centerPanel) {
        ...
        if (self.state == SlidingPanelCenterVisible) {
            self.visiblePanel = _centerPanel;
        }
    }
    if (self.isViewLoaded && self.state == SlidingPanelCenterVisible) {
        ...
        SlidingPanelState previousState = self.state;
        self.state = SlidingPanelCenterVisible;
        [UIView animateWithDuration:0.2f animations:^{
            if (self.bounceOnCenterPanelChange) {
                // first move the centerPanel offscreen
                CGFloat x = (previousState == SlidingPanelLeftVisible) ? self.view.bounds.size.width : -self.view.bounds.size.width;
...
- (void)_swapCenter:(UIViewController *)previous previousState:(SlidingPanelState)previousState with:(UIViewController *)next {
    if (previous != next) {
...
- (void)setLeftPanel:(UIViewController *)leftPanel {
    if (leftPanel != _leftPanel) {
        ...
        if (self.state == SlidingPanelLeftVisible) {
...
- (void)setRightPanel:(UIViewController *)rightPanel {
    if (rightPanel != _rightPanel) {
        ...
        if (self.state == SlidingPanelRightVisible) {
...
- (void)_handlePan:(UIGestureRecognizer *)sender {
    ...
    if ([sender isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (self.style == SlidingPanelMultipleActive || self.pushesSidePanels) {
...
- (CGFloat)_correctMovement:(CGFloat)movement {
    ...
    } else if (self.state == SlidingPanelRightVisible && !self.allowRightOverpan) {
        if (position < -self.rightVisibleWidth) {
            return 0.0f;
        } else if ((self.style == SlidingPanelMultipleActive || self.pushesSidePanels) && position > 0.0f) {
    ...
    } else if (self.state == SlidingPanelLeftVisible  && !self.allowLeftOverpan) {
...
- (void)_loadCenterPanelWithPreviousState:(SlidingPanelState)previousState {
    [self _placeButtonForLeftPanel];
...
- (void)_showLeftPanel:(BOOL)animated bounce:(BOOL)shouldBounce {
    self.state = SlidingPanelLeftVisible;
    [self _loadLeftPanel];
    ...
    if (self.style == SlidingPanelSingleActive) {
...
- (void)_showRightPanel:(BOOL)animated bounce:(BOOL)shouldBounce {
    self.state = SlidingPanelRightVisible;
    [self _loadRightPanel];
...
- (void)toggleLeftPanel:(__unused id)sender {
    if (self.state == SlidingPanelLeftVisible) {
        [self _showCenterPanel:YES bounce:NO];
    } else if (self.state == SlidingPanelCenterVisible) {
        [self _showLeftPanel:YES bounce:NO];
    }
}
...
- (void)setCenterPanelHidden:(BOOL)centerPanelHidden animated:(BOOL)animated duration:(NSTimeInterval) duration {
    if (centerPanelHidden != _centerPanelHidden && self.state != SlidingPanelCenterVisible) {
